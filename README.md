...
The Discovery worker also **starts paused on boot** and runs on `Workers:DiscoveryIntervalSeconds` (default daily). Use the **`/channels`** page to add a channel (a `UC…` id, a `/channel/` URL, or an `@handle`), list / remove channels, resume the worker, or **poll now** for an on-demand poll. A poll lists each channel's recent uploads (id **and title**, in one `playlistItems.list` call — same quota), enqueues only the ones with no existing `Video` row to the processing queue (persisting the title so a video reads by name before it's processed), and stamps `LastPolledUtc` — no transcript or AI work. Every automated test stubs the YouTube API; a real poll consumes YouTube quota.
